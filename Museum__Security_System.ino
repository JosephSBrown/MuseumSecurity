...
void setup() 
{
  ...
  myservo.attach(49);
  pinMode(44, INPUT);
  ogtemp = round(dht.readTemperature());
}
...
void TemperatureScreen() 
{
  ...
  int temp = round(t);
  int temptwo = ogtemp + 1;
  int tempfive = ogtemp + 4;
  int temptwod = ogtemp - 1;
  int tempfived = ogtemp - 4;
  ...
  if  (temp > temptwo || temp < temptwod)
  {
      temperaturechange();
  }

  else if  (temp > tempfive || temp < tempfived)
  {
      temperaturechange();
      tone(3,440,1000);
  }
